...
function renderizarVistaBienvenida() { 
    ...
    if (!contenedorPrincipal) { // contenedorPrincipal es global de globals.js
    ...
    // YA NO SE LLAMA A asignarEventListenersGlobales() DESDE AQUÍ
    
    cambiarVista(null, 'vista-bienvenida'); // cambiarVista de ui_navigation.js
}

function renderizarListaDashboard(divId, libros, tipoLista) {
    // ... (Misma función renderizarListaDashboard que tenías en libros_ui.js o ui_render_views.js)
...
function renderizarDashboard() {
    // ... (Misma función renderizarDashboard que tenías en libros_ui.js o ui_render_views.js)
    // Esta función DEBE llamar a asignarEventListenersLibros() DESPUÉS de crear el HTML de las listas de libros.
    ...
    cargarMisLibrosEnPrestamo(currentUser.id).then(libros => { renderizarListaDashboard('mis-libros-en-prestamo', libros, 'prestadosPorMi'); asignarEventListenersLibros(); }); // Asume globales
    cargarLibrosQueMePrestaron(currentUser.id).then(libros => { renderizarListaDashboard('libros-que-me-prestaron', libros, 'prestadosAMi'); }); // No necesita listeners especiales aquí por ahora
    cargarYMostrarLibros(); // Asume global
}