...
function renderizarVistaBienvenida() { 
    ...
    // NO se llama a asignarEventListenersGlobales() desde aquí. Se hace en main.js -> appInit().
    
    cambiarVista(null, 'vista-bienvenida'); 
}

function renderizarListaDashboard(divId, libros, tipoLista) {
    const div = document.getElementById(divId); if (!div) { console.error(`Div ${divId} no encontrado para lista dashboard`); return; }
    div.innerHTML = '';
    ...
    libros.forEach(libro => {
        ...
        if (tipoLista === 'prestadosPorMi') { 
            ...
            botonHTML = `<button class="btn-marcar-devuelto boton-menu-header" data-libro-id="${libro.id}">Marcar Devuelto</button>`; 
...
function renderizarDashboard() {
    ...
    if (!dashboardView) { console.error("DEBUG: ui_render_views.js - Div 'vista-dashboard' no encontrado."); renderizarVistaBienvenida(); return; } // Llama a renderizar todo si falta
    ...
    dashboardView.innerHTML = `
        ...
        <button id="btn-ir-anadir-libro" class="boton-menu-header">Cargar Libro</button>
        <hr>
    ...
    cargarMisLibrosEnPrestamo(currentUser.id).then(libros => { 
        ...
        asignarEventListenersLibros(); // Asignar listeners a los botones de esta lista
    });
    cargarLibrosQueMePrestaron(currentUser.id).then(libros => { 
        ...
        // No se necesitan listeners especiales aquí por ahora, pero si los hubiera, se llamarían aquí.
    });
    cargarYMostrarLibros(); // Para la lista general
}